...
- [X] git 仓库建好并完成首次 commit
- [X] `java -XX:+PrintFlagsFinal -version` 能跑通，确认默认收集器是 G1
- [X] `javap -c/-v/-l` 三个输出都看过，笔记里回答了 3 个观察问题
- [X] `jps` 能看到 Java 进程 PID
- [X] 笔记完成：用自己的话写"一次编写到处运行到底指什么"（150 字）
  “一次编写，到处运行”指的是 Java 源码只需编译一次，生成与平台无关的字节码。不同系统上安装对应的 JVM，由 JVM 把字节码解释执行，或用 JIT 即时编译成当前平台的机器码。这样，同一份 .class 文件就能在 Windows、Linux、macOS 上运行。JVM 屏蔽了操作系统和硬件的差异，程序员不用为每个平台重新编译，这就是“一次编写，到处运行”的含义。
